...
## 添加配件成功后列表不显示新配件问题修复
- [x] 检查AddPart.tsx中的成功跳转逻辑
- [x] 检查PartsNew.tsx中的列表查询和缓存失效
- [x] 验证数据库中配件是否成功添加
- [x] 修复列表刷新问题：在createPart函数中显式设置isArchived为false
- [x] 测试添加配件后列表自动显示新配件
...
## 真实数据导入（NHTSA API + 示例配件数据）
- [x] 从NHTSA官方API获取车辆品牌/型号数据（30个品牌、2163个型号）
- [x] 生成508个示例配件数据（含真实配件号格式、制造商、价格）
- [x] 生成11个标准分类、93个配件分组
- [x] 编写并运行数据库导入脚本（server/seed.ts）
- [x] 修正车辆型号名称映射（fix_fitments.py），提升适配关系覆盖率
- [x] 数据库现有：30品牌、2163型号、508配件、93分组、2756条适配关系
- [x] 验证TOYOTA RAV4等车型的配件查询结果正常
